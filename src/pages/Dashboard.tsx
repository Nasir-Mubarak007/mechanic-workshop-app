...
//         setTodaysAppointments(todayAppts);
//         setUpcomingAppointments(upcomingAppts);
//         console.log("API /appointments response:", todayAppts);
...
setServices(allServices);
    ...
    const todayAppts = await fetchTodaysAppointments();
    const upcomingAppts = await fetchUpcomingAppointments();
    console.log("Today Appts raw:", todayAppts);
    setTodaysAppointments(todayAppts);
    setUpcomingAppointments(upcomingAppts);
...
<>
  {/* Top stats cards */} 
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6"> <Card className="transform transition-transform hover:scale-105"> <div className="flex items-center"> <div className="p-3 rounded-full bg-red-100 text-red-700"> <Wrench size={24} /> </div> <div className="ml-4"> <p className="text-sm font-medium text-gray-500">Services Today</p> <p className="text-2xl font-semibold text-gray-900">{stats.totalToday}</p> </div> </div> </Card>
        ...
        >
          <div className="overflow-x-auto">
            {todayJobs.length > 0 ? (
                ...
                <tbody className="divide-y divide-gray-200">
                  {todayJobs.slice(0, 5).map((job) => (
                    <tr key={job._id} className="hover:bg-red-50">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500">
                        {job.services.length > 1
                          ? `${job.services[0]?.serviceName || "Service"} +${
                              job.services.length - 1
                            } more`
                          : job.services[0]?.serviceName || "Service"}
                      </td>
              ...
              <div className="text-center py-36 text-gray-500">No jobs recorded today</div>
            )}
          </div>
        </Card>
