...
const ServiceCalendar: React.FC = () => {
  ...
  const loadAppointments = async () => {
    setLoading(true);
    try {
      const allAppointments = await fetchScheduledServices();
      setAppointments(allAppointments);

      const today = await fetchTodaysAppointments();
  ...
  const handleStatusUpdate = async (appointmentId: string, newStatus: AppointmentStatus) => {
    try {
      await updateScheduleStatus(appointmentId, newStatus);
  ...
  const handleDeleteAppointment = async (appointmentId: string) => {
    if (window.confirm("Are you sure you want to delete this appointment?")) {
      try {
        await deleteSchedule(appointmentId);
        await loadAppointments();
  ...
  const getAppointmentsForDate = (date: Date): ScheduledService[] =>
    appointments.filter((a) => isSameDay(parseISO(a.scheduledDate), date));
              ...
              >
                ...
                <div className="space-y-1">
                  {dayAppointments.slice(0, 3).map((a) => (
                    <div
                      key={a._id}
                      className={`text-xs p-1 rounded border ${getStatusColor(a.status)}`}
                    >
                      <div className="font-medium truncate">{a.customerName}</div>
                      <div className="truncate">
                        {format(parseISO(a.scheduledDate), "HH:mm")}
                      </div>
                    </div>
                  ))}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-full ${getStatusColor(a.status)}`}>
            ...
            {upcomingAppointments.map((a) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">
                      {format(parseISO(a.scheduledDate), "MMM dd, HH:mm")}
                    </p>
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(
                        a.status
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-gray-900">{a.customerName}</h4>
                      <p className="text-sm text-gray-500">{a.serviceType}</p>
                      <p className="text-xs text-gray-400">Created by {a.createdByName}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">
                      {format(parseISO(a.scheduledDate), "MMM dd, yyyy HH:mm")}
                    </p>
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(
                        a.status
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <Spinner /> : viewMode === "calendar" ? renderCalendarView() : renderListView()}
            ...
            <div className="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div
                    className={`mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-10 sm:w-10 ${getStatusColor(
                    ...
                    <div className="mt-4">
                      <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                        ...
                        <div className="sm:col-span-2">
                          <dt className="text-sm font-medium text-gray-500 flex items-center">
                            <Car size={16} className="mr-1" /> Vehicle
                          </dt>
                          <dd className="mt-1 text-sm text-gray-900">
                            {selectedAppointment.carDetails}
                          </dd>
                        </div>
                        <div className="sm:col-span-1">
                        ...
                        <div className="sm:col-span-1">
                          <dt className="text-sm font-medium text-gray-500 flex items-center">
                            <Clock size={16} className="mr-1" /> Date & Time
                          </dt>
                          <dd className="mt-1 text-sm text-gray-900">
                  ...
                  {selectedAppointment.status === "scheduled" && (
                    <>
                      ...
                      <Button
                        ...
                        icon={XCircle}
                        onClick={() =>
                          handleStatusUpdate(selectedAppointment._id, "missed")
                        }
                      >
                        Mark Missed
                      </Button>
...
//   };
//   fetchData();
// }, [appointments]); // reload when appointments change
...
//   const renderListView = () => {
//     const todaysAppointments = fetchTodaysAppointments();
//     const upcomingAppointments = fetchUpcomingAppointments();
...
//                         size="sm"
//                         icon={XCircle}
//                         onClick={() => handleStatusUpdate(selectedAppointment._id, 'missed')}
//                       >
//                         Mark Missed
//                       </Button>
//                       <Button
